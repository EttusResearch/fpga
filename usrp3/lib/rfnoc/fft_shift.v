...
(
  ...
  reg [MAX_FFT_SIZE_LOG2-1:0] fft_size, fft_size_minus_2, fft_shift_mask;
    ...
    if (reset) begin
      ...
      if (ping_pong) begin
        ...
        if (pong_loaded & o_tready & o_tvalid) begin
          ...
          if (pong_rd_addr == fft_size_minus_2) begin
            pong_tlast = 1'b1;
          end
        end
      ...
      // Same as above, just ping / pong switched
        ...
        if (ping_loaded & o_tready & o_tvalid) begin
          ...
          if (ping_rd_addr == fft_size_minus_2) begin
            ping_tlast = 1'b1;
          end
        end
        if (i_tvalid & i_tready & i_tlast) begin
