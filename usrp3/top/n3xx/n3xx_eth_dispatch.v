...
module n3xx_eth_dispatch #(
  parameter BASE=0,
  parameter DROP_UNKNOWN_MAC = 1,
  parameter AWIDTH = 8
  )(
  ...
  setting_reg #(.my_addr(REG_FWD_CTRL), .awidth(AWIDTH), .width(2)) sr_forward_ctrl
          ...
          case(state)
              ...
              CLASSIFY_PACKET: begin
                  ...
                  if (is_eth_type_ipv4 && is_ipv4_proto_icmp && is_icmp_no_fwd) begin
                      ...
                      header_ram_addr <= DROP_UNKNOWN_MAC ? HEADER_RAM_SIZE - 1 : 0;
                      state <= DROP_UNKNOWN_MAC ? DROP_PACKET : FORWARD_CPU;
  ...
  fix_short_packet fifo_short_packet_inst (.clk(clk), .reset(reset), .clear(clear),
